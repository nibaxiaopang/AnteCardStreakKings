...
class AnteCardSolitaireView: UIView {
    ...
    var cardToLayerDictionary : [AnteCard : AnteCardLayer]!
    
    var draggingCardLayer : AnteCardLayer? = nil // card layer dragged (nil => no drag)
    var draggingFan : [AnteCard]? = nil  // fan of cards dragged
    var touchStartPoint : CGPoint = CGPoint.zero
    var touchStartLayerPosition : CGPoint = CGPoint.zero
    
    lazy var solitaire : AnteCardSolitaire!  = { // reference to model in app delegate
    ...
    override func awakeFromNib() {
        ...
        stockLayer.backgroundColor =  UIColor(named: "appColor_black 1")?.cgColor
        
        self.layer.addSublayer(stockLayer)
        
        wasteLayer = CALayer()
        wasteLayer.name = "waste"
        wasteLayer.backgroundColor =  UIColor(named: "appColor_black 1")?.cgColor
        wasteLayer.cornerRadius = 8
        self.layer.addSublayer(wasteLayer)
        
        let foundationColor =  UIColor(named: "appColor_black 1")?.cgColor// UIColor(red: 0.0, green: 0.0, blue: 0.5, alpha: 0.3)
        foundationLayers = []
        for i in 0 ..< 4 {
        ...
        let tableauColor = UIColor(named: "appColor_black 1")?.cgColor
        tableauLayers = []
        for i in 0 ..< 7 {
        ...
        let deck = AnteCard.deck()
        cardToLayerDictionary = [:]
        for card in deck {
            let cardLayer = AnteCardLayer(card: card)
            cardLayer.name = "card"
    ...
    //
    func moveCardsToPosition(_ cards: [AnteCard], position : CGPoint, animate : Bool) {
    ...
    func moveCardLayerToTop(_ cardLayer : AnteCardLayer) {
    ...
    func animateDeal( _ cardLayers : [AnteCardLayer]) {
    ...
    func undoMultiCard(_ cards : [AnteCard]) {
        undoManager?.registerUndo(withTarget: self, handler: { me in
            me.multiCardDeal()
        })
        undoManager?.setActionName("deal cards")
    ...
    func flipCard(_ card : AnteCard, faceUp up : Bool) {
        undoManager?.registerUndo(withTarget: self, handler: {me in
    ...
    func dropCard(_ card : AnteCard, onFoundation i : Int) {
        let cardLayer = cardToLayerDictionary[card]!
        moveCardLayerToTop(cardLayer)
    ...
    func undoDropCard(_ card: AnteCard, fromStack source: AnteCardStack, onFoundation i : Int) {
    ...
    func dropCard(_ card : AnteCard, onTableau i : Int) {
        let cardLayer = cardToLayerDictionary[card]!
    ...
    func undoDropCard(_ card: AnteCard, fromStack source: AnteCardStack, onTableau i : Int) {
    ...
    func dropFan(_ cards : [AnteCard], onTableau i : Int) {
    ...
    func undoDropFan(_ cards : [AnteCard], fromStack source: AnteCardStack, onTableau i : Int) {
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if let layer = layer {
            if layer.name == "card" {
                let cardLayer = layer as! AnteCardLayer
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        if let dragLayer = draggingCardLayer {
            ...
            if let fan = draggingFan {
                for i in 0 ..< 7 {
                    let topCard = solitaire.tableau[i].isEmpty ? nil : solitaire.tableau[i].last
                    var topCardLayer : AnteCardLayer? = nil
                    var targetFrame : CGRect
                    if let topCard = topCard {
